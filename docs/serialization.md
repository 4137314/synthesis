...
The reference codec uses named record fields and tagged inductive alternatives from
Lean's JSON derivation. Declaration and argument lists retain their order: parameter
...
match those fixtures; changing Lean derivation output requires a new format version,
...
no kernel theorem about the JSON parser/derived decoder. `Design.Encoding` and
`AttributeCodec` separately require kernel-checked exactness for typed domain data.
Content hashing is not part of identity and is not implemented; a future scheme must
hash a specified format version and must not replace scoped entity identity.
